...
Importe este repositório na Vercel usando o preset **Next.js**. A raiz do repositório já é a raiz do projeto.

- Instalação: `pnpm install --frozen-lockfile`
- Build: `pnpm build`
- Variáveis de ambiente obrigatórias: nenhuma

O domínio de produção informado pela Vercel é usado automaticamente nos metadados. Para definir um domínio próprio antes do deploy, preencha `seo.url` em `data/site.ts`.

## Desenvolvimento

Requer Node.js 20.9 ou superior e pnpm. Execute `pnpm install` e `pnpm dev`.
